...
use Modules\Product\Http\Requests\v1\ProductImage\ProductImageStoreRequest;
use Modules\Product\Http\Requests\v1\ProductImage\ProductImageUpdateRequest;
...
class ProductImageController extends CoreController
{
    ...
    {
        $data = $this->service->findByProductId($productId)->get();
    ...
    {
        $result = $this->service->getImage($id);
        return fileResponse($result);
    }

    public function getThumbnail($id)
    {
        $result = $this->service->getThumbnail($id);
        return fileResponse($result);
    }
}
